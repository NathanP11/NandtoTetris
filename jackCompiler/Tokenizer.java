...
public class Tokenizer {
	...
	public Tokenizer(File f, String fileName) {
		...
		keywordMap = new HashSet<String>();
		symbolMap = new HashSet<String>();
		whiteMap = new HashSet<String>();

		keywordMap.add("class");
		keywordMap.add("constructor");
		keywordMap.add("function");
		keywordMap.add("method");
		keywordMap.add("field");
		keywordMap.add("static");
		keywordMap.add("var");
		keywordMap.add("int");
		keywordMap.add("char");
		...
		index = 0;
		curr = null; // reset the tokenizer after making the xml file
	}
...
//=============================================================================================
	...
	public void advance() {
			...
			if (index + 1 == file.length()) {
				...
				if (currChar.equals("\"")) { // grab full string
					...
					if (whiteMap.contains(currChar)) { // currChar.equals("\n") ||
														// currChar.equals(System.lineSeparator()) ||
														// currChar.equals("\r") ) {
						index++;
					} else {
						...
						if (symbolMap.contains(currChar)) {
							type = "symbol";
							isToken = true;
						}
		...
		if (keywordMap.contains(sb.toString())) {
			type = "keyword";
		}
		if (type.equals("")) {
			type = "identifier";
		...
		// System.out.println("Processing Token: " + value() + " type: " + type() );
	...
	private void removeComments() {
		while (file.indexOf("/*") != -1) {
			file.delete(file.indexOf("/*"), file.indexOf("*/") + 2);
		}
		...
		while (file.indexOf("//") != -1) {
			...
			if (stop < 0) {
				stop = file.indexOf("\r", start + 1);
			}
			stop += 1;
			// String deleteThis = file.substring(start,stop);
			file.delete(start, stop);
